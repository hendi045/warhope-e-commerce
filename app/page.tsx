...
import { Heart, CheckCircle2, Mail, ArrowRight, Sparkles, ShieldCheck, Truck, Star, ArrowDownAZ, ArrowUpZA, MessageCircle } from 'lucide-react';
...
export default function Home() {
  ...
  return (
    <main className="pt-4 md:pt-8 pb-16 max-w-7xl mx-auto px-4 sm:px-6 min-h-screen bg-background relative">
      ...
      <section className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-6 mb-16">
        <div className="md:col-span-2 relative bg-slate-200 dark:bg-slate-800 rounded-4xl overflow-hidden min-h-100 md:min-h-125 group">
          {/* PERUBAHAN: Menggunakan gambar dari folder public/assets */}
          {/* eslint-disable-next-line @next/next/no-img-element */}
          <img 
            src="/assets/warhope-og-banner.png" 
            alt="Warhope Apparel Lifestyle" 
        ...
        <div className="flex flex-col gap-4 md:gap-6">
          <div className="bg-slate-900 rounded-4xl p-8 flex-1 flex flex-col justify-center relative overflow-hidden group">
            ...
            <p className="text-slate-400 text-sm font-medium mb-6">Untuk pembelian pertama apparel eksklusif kami.</p>
          ...
          <div className="bg-slate-100 dark:bg-slate-800 rounded-4xl relative overflow-hidden flex-1 group">
            {/* PERUBAHAN: Menggunakan gambar lokal yang sama untuk banner kanan bawah */}
            {/* eslint-disable-next-line @next/next/no-img-element */}
            <img 
              src="/assets/warhope-og-banner.png" 
            ...
            <div className="absolute inset-0 flex items-center justify-center">
              <h3 className="text-2xl font-black text-white tracking-widest uppercase shadow-sm">Katalog Utama</h3>
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-20">
        <div className="bg-white dark:bg-slate-800/50 border border-slate-100 dark:border-slate-800 rounded-3xl p-6 flex items-center gap-4 shadow-sm transition-colors">
          ...
          <div>
            <h4 className="font-bold text-foreground text-sm">Aman & Terpercaya</h4>
            <p className="text-xs text-foreground/60 mt-1">Ke seluruh wilayah Indonesia</p>
          </div>
        </div>
        <div className="bg-white dark:bg-slate-800/50 border border-slate-100 dark:border-slate-800 rounded-3xl p-6 flex items-center gap-4 shadow-sm transition-colors">
          <div className="w-12 h-12 bg-slate-50 dark:bg-slate-800 rounded-full flex items-center justify-center shrink-0">
          ...
          <div>
            ...
            <p className="text-xs text-foreground/60 mt-1">Material premium & tahan lama</p>
          </div>
        </div>
        <div className="bg-white dark:bg-slate-800/50 border border-slate-100 dark:border-slate-800 rounded-3xl p-6 flex items-center gap-4 shadow-sm transition-colors">
          <div className="w-12 h-12 bg-slate-50 dark:bg-slate-800 rounded-full flex items-center justify-center shrink-0">
          ...
          <div>
            ...
            <p className="text-xs text-foreground/60 mt-1">Gaya urban & streetwear modern</p>
          </div>
        </div>
      </section>
      ...
      <section id="katalog" className="scroll-mt-32">
        ...
        <div className="flex flex-col lg:flex-row gap-8">
          ...
          {/* GRID PRODUK (2 Kolom di Mobile, 3 Kolom di PC) */}
        ...
        <div className="bg-slate-100 dark:bg-slate-800/50 rounded-4xl p-8 md:p-16 relative overflow-hidden flex flex-col md:flex-row items-center gap-12 border border-transparent dark:border-slate-800">
          ...
          <div className="relative z-10 flex-1 space-y-6">
            ...
            <p className="text-foreground/70 font-medium text-lg max-w-md">
              Dapatkan akses awal untuk rilisan terbatas, penawaran eksklusif, dan diskon 10% untuk pesanan pertama Anda.
            </p>
      ...
      {/* --- FLOATING WHATSAPP BUTTON (CUSTOMER SUPPORT) --- */}
      <a 
        href="https://wa.me/6281288111154?text=Halo%20Admin%20Warhope,%20saya%20ingin%20bertanya%20seputar%20produk%20Anda." 
        target="_blank" 
        rel="noopener noreferrer"
        className="fixed bottom-6 right-6 sm:bottom-8 sm:right-8 z-50 bg-[#25D366] text-white p-4 rounded-full shadow-xl hover:scale-110 hover:shadow-2xl transition-all duration-300 flex items-center justify-center group"
        aria-label="Chat WhatsApp CS"
      >
        <MessageCircle className="w-6 h-6 sm:w-7 sm:h-7 relative z-10" />
        
        {/* Tooltip Hover */}
        <span className="absolute right-full mr-4 bg-white dark:bg-slate-800 text-slate-800 dark:text-white text-xs font-bold px-3 py-2 rounded-xl shadow-lg opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap pointer-events-none hidden sm:block">
          Tanya CS Kami
        </span>
        
        {/* Efek Denyut (Ping) Penarik Perhatian */}
        <span className="absolute flex h-full w-full rounded-full bg-[#25D366] opacity-40 animate-ping"></span>
      </a>
